...
def calculate_similarity(DATASET, MODEL, DETAILED_CONCEPT_PROMPTS):
    ...
    for im in img_embeddings.keys():
        ...
        similarity_scores = []
        for concept in DETAILED_CONCEPT_PROMPTS.keys():
            # Load text embeddings of detailed concepts
            text_feats = torch.from_numpy(
                np.load(f"text_embeddings/detailed_concept_embeddings_{concept}.npy")).unsqueeze(0)
...
if __name__ == '__main__':
    ...
    for idx, m in enumerate(MODEL):
        results = calculate_similarity(DATASET, m, DETAILED_CONCEPT_PROMPTS)
    ...
    np.save("results_PH2_GPT-CBM.npy", metrics_dict)
