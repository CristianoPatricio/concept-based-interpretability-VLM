...
def calculate_similarity(DATASET, MODEL, CLASS_LABELS_PROMPTS):
    ...
    img_embeddings = np.load(f"img_embeddings/image_embeddings_{DATASET}_MONET_{MODEL}_Segmented_Norm.npy",
    ...
    for im in img_embeddings.keys():
        ...
        for disease_label in CLASS_LABELS_PROMPTS.keys():
            ...
            text_feats = torch.from_numpy(
                np.load(f"text_embeddings/class_label_embeddings_{disease_label}.npy")).unsqueeze(0)
