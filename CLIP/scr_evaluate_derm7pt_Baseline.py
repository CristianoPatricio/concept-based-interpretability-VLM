...
def calculate_similarity(MODEL, CLASS_LABELS_PROMPTS, SEED):
    ...
    img_embeddings = np.load(f"output/{MODEL}/image_embeddings/image_embeddings__derm7pt_{MODEL}-Fine-Tuned_{SEED}.npy",
                             allow_pickle=True).item()

    # Load reference embeddings
    reference_embeddings = torch.from_numpy(np.load(
        f"output/{MODEL}/text_embeddings/reference_embeddings__derm7pt_{MODEL}-Fine-Tuned_{SEED}.npy")).unsqueeze(0)
    ...
    for im in img_embeddings.keys():
        ...
        for disease_label in CLASS_LABELS_PROMPTS.keys():
            ...
            text_feats = torch.from_numpy(np.load(
                f"output/{MODEL}/text_embeddings/class_label_embeddings_{disease_label}_derm7pt_{MODEL}-Fine-Tuned_{SEED}.npy")).unsqueeze(
                0)
...
if __name__ == '__main__':
    ...
    np.save("results_derm7pt_baseline.npy", metrics_dict)
