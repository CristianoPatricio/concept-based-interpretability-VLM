...
def calculate_similarity(MODEL, CONCEPT_PROMPTS, SEED):
    ...
    for im in img_embeddings.keys():
        ...
        for disease_label in CONCEPT_PROMPTS.keys():
            ...
            text_feats = torch.from_numpy(np.load(
                f"output/{MODEL}/text_embeddings/concept_embeddings_{disease_label}_{DATASET}_{MODEL}-Fine-Tuned_{SEED}.npy")).unsqueeze(
                0)
...
def evaluate(results_dict):
    ...
    for im in results_dict.keys():
        ...
        if str(im) in test_images:
            ...
            y_pred.append(1 if results_dict[im] > optimal_threshold else 0)
...
if __name__ == '__main__':
    ...
    cbm_42 = [0.710, 0.740, 0.712, 0.661, 0.676, 0.724]
    ...
    for idx, m in enumerate(MODEL):
        ...
        for s in SEED:
            if s == 42:
                bacc_acum.append(cbm_42[idx])
            else:
                results = calculate_similarity(MODEL=m, CONCEPT_PROMPTS=CONCEPT_PROMPTS, SEED=s)
    ...
    #print(metrics_dict)
    np.save("results_ISIC_2018_CBM.npy", metrics_dict)
